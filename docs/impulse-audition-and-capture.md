...
**Auto-normalize output** is an optional post-capture action. It measures the
largest absolute sample across both captured channels and changes the visible,
persisted **Stereo Output / Gain** so that the measured peak targets `-0.1 dBFS`.
The calculation accounts for the output gain that was active during capture,
is refused for silence, and is capped at `100x` (`+40 dB`). The edit is undoable
and marks a factory patch as customized. Temporary audition overlays, A/B
auditions, and tuning previews must be cleared first so they cannot influence a
saved normalization decision.
...
- Capture data is taken after Stereo Output Gain but before Wet and Dry Gain, so
  the explicit patch-boundary gain is measured while audition-level changes do
  not alter analysis samples.
...
Current output-normalization UI evidence is stored in
[`artifacts/ui/output-normalization/`](../artifacts/ui/output-normalization/).
