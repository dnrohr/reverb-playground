...
Dropping a second cable on an occupied audio input does not silently replace or sum it. A dialog offers three explicit outcomes:

- **Insert +** creates a Sum block, moves the old source to `in-a`, sends the new source to `in-b`, and connects the Sum output to the original target.
