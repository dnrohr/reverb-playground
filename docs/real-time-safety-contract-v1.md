...
The M3.3 acyclic runtime implements off-thread preparation and atomic pointer publication. Its audio callback only marks the processing interval, loads the active raw pointer, and processes prepared spans. The publishing control thread—not the callback—waits for any retired runtime to become unobservable before destroying it. Invalid compilation never exchanges the pointer. Feedback scheduling and the transition mute/crossfade policy remain subsequent compiler integration work.
