...
The M3.3/M3.4 graph runtime implements off-thread preparation and atomic pointer publication for feed-forward and feedback graphs. Its audio callback only marks the processing interval, loads the active raw pointer, and processes prepared spans. The publishing control thread—not the callback—waits for any retired runtime to become unobservable before destroying it. Invalid compilation never exchanges the pointer. Feedback uses a bounded Delay-read, current-sample-evaluate, Delay-write sequence; the transition mute/crossfade policy remains subsequent compiler integration work.
