...
**Analyze / Diagnostics** opens the Analyze tab without covering graph controls. Static workload estimates, measured live CPU/clipping, and exact prepared delay memory are labeled by evidence type. A numerical safety latch opens Analyze automatically, identifies the immutable offending graph revision, and leaves editing plus Undo available. **Recover Audio** is enabled only while safety-muted and clears the latch explicitly after the user has reduced or undone the risky setting.
...
Hosted editors prefer 1280 by 800 logical pixels, while the standalone starts maximized. The embedded WebView receives the exact remaining logical parent bounds below the intentional 88-pixel native audition strip; JUCE then converts them once to WebView2 physical controller pixels, so the schematic fills that region at 100%, 125%, and 150% scaling.
