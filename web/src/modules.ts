...
import { pitchShiftMaximumSemitones, pitchShiftMinimumSemitones } from './pitchShiftContract';

export type ModuleType = 'stereo-input' | 'stereo-output' | 'gain' | 'sum' | 'delay' | 'allpass' | 'lowpass' | 'pitch-shift' | 'macro' | 'lfo' | 'control-map' | 'envelope-follower' | 'hold-gate';
...
export const moduleDefinitions: ModuleDefinition[] = [
  ...
  { type: 'stereo-output', label: 'Stereo Output', role: 'io', ports: [audioIn('in-l'), audioIn('in-r')], parameters: [staticParameter('gain', 1, 'linear', 0, 100, 0.01)] },
  ...
  { type: 'pitch-shift', label: 'Pitch Shift', role: 'pitch', ports: [audioIn(), controlIn('semitones-mod'), controlIn('grain-mod'), controlIn('overlap-mod'), audioOut()], parameters: [parameter('semitones', 12, 'semitones', pitchShiftMinimumSemitones, pitchShiftMaximumSemitones, 0.01, 12), parameter('grain', 60, 'milliseconds', 20, 120, 0.1, 20), parameter('overlap', 0.5, 'normalized', 0.1, 1, 0.01, 0.25), staticParameter('direction', 0, 'direction', 0, 1, 1), staticParameter('phase', 0, 'cycles', 0, 0.999, 0.001)] },
