...
export function PatchNode({ data, selected }: NodeProps & { data: PatchNodeData }) {
  ...
  if (isHierarchyParent && hierarchy) return (
    <article className={`patch-node hierarchy-parent${selected ? ' is-selected' : ''}`}
      aria-label={`${data.label}, ${inputs.length} inputs, ${outputs.length} outputs. Double-click to open nested schematic.`}>
      {inputs.map((port, index) => <span className="hierarchy-port-anchor hierarchy-port-in" key={port.id} style={{ top: topFor(index, inputs.length) }}>
        <Handle className={`port port-${port.signal}`} id={port.id} position={Position.Left} type="target" />
        <i>{(hierarchy.ports.find((item) => item.id === port.id)?.name ?? port.id).replace(/^IN\s*/i, '')}</i>
      </span>)}
      {outputs.map((port, index) => <span className="hierarchy-port-anchor hierarchy-port-out" key={port.id} style={{ top: topFor(index, outputs.length) }}>
        <i>{(hierarchy.ports.find((item) => item.id === port.id)?.name ?? port.id).replace(/^OUT\s*/i, '')}</i>
        <Handle className={`port port-${port.signal}`} id={port.id} position={Position.Right} type="source" />
      </span>)}
      <span className="hierarchy-side-label hierarchy-side-in">IN</span>
      <span className="hierarchy-side-label hierarchy-side-out">OUT</span>
  ...
  return (
    <article className={`patch-node role-${data.role}${selected ? ' is-selected' : ''}`} aria-label={`${visibleModuleLabel(data)} ${moduleSignalBadge(data)}. ${vocabularyFor(data)?.audibleRole ?? data.type}`}>
      {inputs.map((port, index) => (
        <span className="hierarchy-port-anchor hierarchy-port-in" key={port.id} style={{ top: topFor(index, inputs.length) }}>
          <Handle className={`port port-${port.signal}`} id={port.id} position={Position.Left} type="target" />
      ...
      {outputs.map((port, index) => (
        <span className="hierarchy-port-anchor hierarchy-port-out" key={port.id} style={{ top: topFor(index, outputs.length) }}>
          {isHierarchyParent && hierarchy ? <i>{hierarchy.ports.find((item) => item.id === port.id)?.name ?? port.id}</i> : null}
          <Handle className={`port port-${port.signal}`} id={port.id} position={Position.Right} type="source" />
        </span>
      ))}
