...
export const moduleVocabulary: Record<ModuleType, ModuleVocabulary> = {
  ...
  'stereo-output': { displayName: 'Stereo Output', signal: '2 MONO AUDIO INPUTS → GAIN → STEREO BOUNDARY', audibleRole: 'Returns explicit left and right channels through one shared output gain.', increasingControl: 'Increasing Gain raises both output channels from silence up to 100× (+40 dB).', purpose: 'Normalizes quiet patches at the visible graph boundary without changing internal feedback.', latencySafety: 'Adds no latency; numerical safety guards still inspect the amplified result.', advancedParameterIds: [] },
