export interface RuntimeDiagnostics {
  ...
  preparedGraph: { nodeCount: number; connectionCount: number; feedbackRegionCount: number; blockWiseRegionCount: number; sampleWiseRegionCount: number; logicalAudioBufferCount: number; logicalSignalCount: number; elidedNonAudioBufferCount: number; physicalAudioBufferCount: number; peakLiveBufferCount: number; bufferBytesSaved: number; inPlaceAliasCount: number; copiesAvoided: number; fusedKernelCount: number; fusedNodeCount: number; simdKernelCount: number; bufferRetentionReasons: { reason: string; signalCount: number }[]; fusionPreventionReasons: { reason: string; signalCount: number }[]; preparedStorageBytes: number; compileTiming: { validationMicroseconds: number; schedulingMicroseconds: number; preparationMicroseconds: number; totalMicroseconds: number; requestToActiveMicroseconds: number } };
...
export function parseRuntimeDiagnostics(value: unknown): RuntimeDiagnostics {
  ...
  if (!Array.isArray(preparedGraph.bufferRetentionReasons)) throw new Error('buffer retention reasons must be an array');
  const bufferRetentionReasons = preparedGraph.bufferRetentionReasons.map((item, index) => {
    const reason = record(item, `buffer retention reason ${index}`);
    return { reason: text(reason.reason, 'buffer retention reason'), signalCount: count(reason.signalCount, 'retained signal count') };
  });
  if (!Array.isArray(preparedGraph.fusionPreventionReasons)) throw new Error('fusion prevention reasons must be an array');
  const fusionPreventionReasons = preparedGraph.fusionPreventionReasons.map((item, index) => {
    const reason = record(item, `fusion prevention reason ${index}`);
    return { reason: text(reason.reason, 'fusion prevention reason'), signalCount: count(reason.signalCount, 'fusion prevention count') };
  });
  ...
  return {
    ...
    preparedGraph: { nodeCount: count(preparedGraph.nodeCount, 'prepared node count'), connectionCount: count(preparedGraph.connectionCount, 'prepared connection count'), feedbackRegionCount: count(preparedGraph.feedbackRegionCount, 'feedback region count'), blockWiseRegionCount: count(preparedGraph.blockWiseRegionCount, 'block-wise region count'), sampleWiseRegionCount: count(preparedGraph.sampleWiseRegionCount, 'sample-wise region count'), logicalAudioBufferCount: count(preparedGraph.logicalAudioBufferCount, 'logical audio buffer count'), logicalSignalCount: count(preparedGraph.logicalSignalCount, 'logical signal count'), elidedNonAudioBufferCount: count(preparedGraph.elidedNonAudioBufferCount, 'elided non-audio buffer count'), physicalAudioBufferCount: count(preparedGraph.physicalAudioBufferCount, 'physical audio buffer count'), peakLiveBufferCount: count(preparedGraph.peakLiveBufferCount, 'peak live buffer count'), bufferBytesSaved: count(preparedGraph.bufferBytesSaved, 'saved buffer bytes'), inPlaceAliasCount: count(preparedGraph.inPlaceAliasCount, 'in-place alias count'), copiesAvoided: count(preparedGraph.copiesAvoided, 'copies avoided'), fusedKernelCount: count(preparedGraph.fusedKernelCount, 'fused kernel count'), fusedNodeCount: count(preparedGraph.fusedNodeCount, 'fused node count'), simdKernelCount: count(preparedGraph.simdKernelCount, 'SIMD kernel count'), bufferRetentionReasons, fusionPreventionReasons, preparedStorageBytes: count(preparedGraph.preparedStorageBytes, 'prepared storage'), compileTiming: { validationMicroseconds: count(compileTiming.validationMicroseconds, 'validation time'), schedulingMicroseconds: count(compileTiming.schedulingMicroseconds, 'scheduling time'), preparationMicroseconds: count(compileTiming.preparationMicroseconds, 'preparation time'), totalMicroseconds: count(compileTiming.totalMicroseconds, 'compile time'), requestToActiveMicroseconds: count(compileTiming.requestToActiveMicroseconds, 'request-to-active time') } },
